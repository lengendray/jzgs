...
public abstract class BarLineChartBase<T extends BarLineScatterCandleData<? extends BarLineScatterCandleRadarDataSet<? extends Entry>>>
    ...
    /** flat indicating if the graph has been drawn */
    protected boolean mDrawGraphFinish = false;
    ...
    protected void onDraw(Canvas canvas) {
		...
		boolean drawGraph = !mDrawGraphFinish;
        
		if (getPhaseX() < 1.0f || getPhaseY() < 1.0f) {
			drawGraph = true;
		}
        
        if(drawGraph){
        	drawGraph(canvas);
        	mDrawGraphFinish = true;
        }
        ...
        if (mHighlightEnabled && mHighLightIndicatorEnabled && valuesToHighlight()){
            ...
            // content-rect
            int clipRestoreCount = mCanvas.save();
            mCanvas.clipRect(mContentRect);
        	
        	drawHighlights();
        	
            // Removes clipping rectangle
        	mCanvas.restoreToCount(clipRestoreCount);
        	
        	Log.i(LOG_TAG, "draw highlights time: " + (System.currentTimeMillis() - starttime) + " ms");
        }
    ...
    private void drawGraph(Canvas canvas){
        ...
        Log.i(LOG_TAG, "draw graph time: " + (System.currentTimeMillis() - starttime) + " ms");
    }
    ...
    protected void calculateOffsets() {
        ...
        if (mLegend.getPosition() == LegendPosition.RIGHT_OF_CHART) {
            ...
            if (mXLabels.getPosition() == XLabelPosition.TOP)
                mLegend.setOffsetTop(mLegendLabelPaint.getTextSize() * 2.5f);
            else {
                mLegend.setOffsetTop(mLegendLabelPaint.getTextSize() * 3.5f);
            }
		}
    ...
    private void drawYLabels(float xPos, float[] positions, float yOffset) {
        ...
        for (int i = 0; i < mYLabels.mEntryCount; i++) {
            ...
            if(mYLabels.getLabelColorFormatter() != null){
            	LabelColorFormatter colorFormatter = mYLabels.getLabelColorFormatter();
				mYLabelPaint.setColor(colorFormatter
						.getFormattedColor(mYLabels.mEntries[i]));
            }
			...
			float f1 = positions[i * 2 + 1];
			float f2 = getHeight() - mOffsetBottom;

			if (Math.abs(f2 - f1) < Utils.convertDpToPixel(2)) {
				continue;
			}
        ...
        for (int i = 0; i < mBorderPositions.length; i++) {
            ...
            switch (mBorderPositions[i]) {
                ...
                case BOTTOM:
                	...
                	mDrawCanvas.drawLine(left, getHeight() - mOffsetBottom, right,
    ...
    protected void drawHorizontalGrid() {
        ...
        for (int i = 0; i < mYLabels.mEntryCount; i++) {
			...
			double f1 = getPixelsForValues(0, mYLabels.mEntries[i]).y;
			double f2 = getHeight() - mOffsetBottom;
